...
export class Game {
  ...
  update() {
    ...
    if (this._vine) {
      ...
      if (onVine && input.jump && this.areaTransTimer === 0) {
        ...
        p.vy = 0;
        p.isJumping = false;
        if (p.y < v.targetTop + 30) {
          if (this.world === 2 && this.world2Level === 0) {
            this._vine = null;
          ...
          } else if (this.world === 5 && this.world5Level === 1) {
            this._vine = null;
            this._startWorld5SubAreaTransition(2);   // 5-2 sky bonus
          } else if (this.world === 6 && this.world6Level === 1) {
            this._vine = null;
            this._startWorld6SubAreaTransition(3);   // 6-2 sky bonus
          }
        }
      }
    }
    ...
    // ── FSM zoneDisableLakitu: past this point the Zubat flies away ──
    if (lvl.lakituStopX && p.x > lvl.lakituStopX) {
      for (const e of lvl.enemies) {
        if (e.type === 'zubat' && !e.fleeing) e.fleeing = true;
      }
    }

    // ── Cannons (FSM Bill Blasters): fire every 270 frames, but never while
    //    the player stands right next to them ──
    for (const c of (lvl.cannons || [])) {
      if (c.flash > 0) c.flash--;
      if (Math.abs((c.x + c.w / 2) - (p.x + p.w / 2)) > 900) continue;   // off-screen
      c.timer--;
      if (c.timer > 0) continue;
      c.timer = 270;
      if (p.x + p.w > c.x - 32 && p.x < c.x + c.w + 32) continue;        // too close
      const dir = (p.x + p.w / 2) < (c.x + c.w / 2) ? -1 : 1;
      const bill = new Enemy(dir < 0 ? c.x - 32 : c.x + c.w, c.y + 28, 'bulletbill');
      bill.vx = 2 * dir;
      lvl.enemies.push(bill);
      c.flash = 10;
    }
    ...
    if (lvl.isSky && !p.dead && p.y > 700 && this.areaTransTimer === 0) {
      ...
      else if (this.world === 5) this._startWorld5SubAreaTransition(0);
      else if (this.world === 6) this._startWorld6SubAreaTransition(0);
          ...
          if (p.x + p.w > pl.x && p.x < pl.x + pl.w &&
            ...
            const callback = () => {
              if (this.world === 2 && pl.leadsToArea !== undefined) {
                ...
                this._handleWorld3PipeEntry(pl);
              } else if (this.world === 4) {
                this._handleWorld4PipeEntry(pl);
      ...
      for (const hp of lvl.hPipeExits) {
        if (p.x + p.w >= hp.x && p.x < hp.x + TILE * 2 &&
          ...
          this._pipeEntry = { timer: 22, dx: 2.5, dy: 0, hPipe: hp, callback: () => {
            ...
            else if (this.world === 3) { this._handleWorld3PipeEntry({ transportId: tid }); }
    ...
    if (this.areaTransTimer > 0) {
      ...
      if (this.areaTransTimer === 0) {
        if (this._pendingWorld1SubArea >= 0) {
          ...
          this._pendingWorld2SubArea = -1;
        } else if (this._pendingWorld3SubArea >= 0) {
          this._doWorld3SubAreaTransition(this._pendingWorld3SubArea);
          this._pendingWorld3SubArea = -1;
        } else if (this._pendingWorld4SubArea >= 0) {
          this._doWorld4SubAreaTransition(this._pendingWorld4SubArea);
          this._pendingWorld4SubArea = -1;
        } else if (this._pendingWorld5SubArea >= 0) {
          this._doWorld5SubAreaTransition(this._pendingWorld5SubArea);
          this._pendingWorld5SubArea = -1;
  ...
  _startWorld3SubAreaTransition(subArea) {
    this._pendingWorld3SubArea = subArea;
    this.areaTransTimer = 40;
  }

  _handleWorld3PipeEntry(pipe) {
    const tid = pipe.transportId;
    if (this.world3Level === 0) {
      // 3-1: transport 3 → underworld; transport 1 (underworld exit) → overworld
      if (tid === 1 || this.world3SubArea === 1) this._startWorld3SubAreaTransition(0);
      else this._startWorld3SubAreaTransition(1);
    }
  }

  _doWorld3SubAreaTransition(subArea) {
    const prevPower = this.player.power;
    const prevSubArea = this.world3SubArea;
    this.world3SubArea = subArea;
    this.level = buildWorld3(this.world3Level, subArea);
    this.r.currentSetting = this.level.setting || 'overworld';
  ...
  _startWorld4SubAreaTransition(subArea) {
    this._pendingWorld4SubArea = subArea;
    this.areaTransTimer = 40;
  }

  _handleWorld4PipeEntry(pipe) {
    const tid = pipe.transportId;
    if (this.world4Level === 0) {
  ...
  _doWorld4SubAreaTransition(subArea) {
    const prevPower = this.player.power;
    const prevSubArea = this.world4SubArea;
    this.world4SubArea = subArea;
    this.level = buildWorld4(this.world4Level, subArea);
    this.r.currentSetting = this.level.setting || 'overworld';
  ...
  _startWorld5SubAreaTransition(subArea) {
    this._pendingWorld5SubArea = subArea;
    this.areaTransTimer = 40;
  }

  _handleWorld5PipeEntry(pipe) {
    const tid = pipe.transportId;
    if (this.world5Level === 0) {
      // 5-1: transport 2 → underworld; transport 1 (its exit) → overworld
      if (tid === 1 || this.world5SubArea === 1) this._startWorld5SubAreaTransition(0);
      else this._startWorld5SubAreaTransition(1);
    } else if (this.world5Level === 1) {
      // 5-2: transport 3 → underwater; transport 1 (its exit) → overworld
      if (tid === 1 || this.world5SubArea === 1) this._startWorld5SubAreaTransition(0);
      else if (tid === 3) this._startWorld5SubAreaTransition(1);
    }
  }

  _doWorld5SubAreaTransition(subArea) {
    const prevPower = this.player.power;
    const prevSubArea = this.world5SubArea;
    this.world5SubArea = subArea;
    this.level = buildWorld5(this.world5Level, subArea);
    this.r.currentSetting = this.level.setting || 'overworld';
  ...
  _startWorld6SubAreaTransition(subArea) {
    this._pendingWorld6SubArea = subArea;
    this.areaTransTimer = 40;
  }

  _handleWorld6PipeEntry(pipe) {
    const tid = pipe.transportId;
    if (this.world6Level !== 1) return;
    // Leaving any 6-2 bonus area → back to the main level, emerging on the
    // pipe that belongs to that bonus
    if (this.world6SubArea !== 0) {
      this._pendingSpawnX = WORLD62_RETURN_X[this.world6SubArea] ?? 80;
      this._startWorld6SubAreaTransition(0);
      return;
    }
    if (tid === 5) this._startWorld6SubAreaTransition(1);       // first underworld
    else if (tid === 6) this._startWorld6SubAreaTransition(2);  // underwater
    else if (tid === 8) this._startWorld6SubAreaTransition(4);  // second underworld
  }

  _doWorld6SubAreaTransition(subArea) {
    const prevPower = this.player.power;
    this.world6SubArea = subArea;
    this.level = buildWorld6(this.world6Level, subArea);
    this.r.currentSetting = this.level.setting || 'overworld';
  ...
  _curLevelIdx() {
    return this.world === 1 ? this.world1Level
